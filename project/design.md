...
- **No arguments prints help.** A bare invocation has no start folder intent, so it shows help and exits 0 rather than scanning the current directory. Listing the current directory is still one keystroke away (`.`). Handled up front in `run` before `parseArgs`.
